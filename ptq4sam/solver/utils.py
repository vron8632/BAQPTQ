...
def parse_config(config_file):
    ...
    # 处理MoRA配置
    if hasattr(config, 'ptq4sam') and hasattr(config.ptq4sam, 'mora_config'):
        mora_cfg = config.ptq4sam.mora_config
        if mora_cfg.enabled:
            logger.info(f'MoRA配置加载: rank={mora_cfg.rank}, alpha={mora_cfg.alpha}, bypass_ratio={mora_cfg.bypass_ratio}')
