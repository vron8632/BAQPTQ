...
def quantize_model(model, config_quant, args):
    ...
    # 准备MoRA配置
    mora_config = None
    if hasattr(config_quant.ptq4sam, 'mora_config') and config_quant.ptq4sam.mora_config.enabled:
        mora_cfg = config_quant.ptq4sam.mora_config
        mora_config = MoRAConfig(
            enabled=mora_cfg.enabled,
            rank=mora_cfg.rank,
            alpha=mora_cfg.alpha,
            target_modules=mora_cfg.target_modules
            # bypass_ratio已废弃，使用直接相加方式
        )
        logger.info(f'启用MoRA旁路量化: rank={mora_cfg.rank}, alpha={mora_cfg.alpha}')
