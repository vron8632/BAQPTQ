...
from ptq4sam.quantization.mora_adapter import MoRAConfig
...
def update_specialized_quantizer_config(base_config, quantizer_name):
    ...
    update_keys = {
        ...
        'diagq':{'quantizer':'DIAGQFakeQuantize',
                 'observer':'LogAvgMSEFastObserver'},  # 融合DFQ+AGQ
        'dfq':{'quantizer':'SimpleDFQFakeQuantize',
               'observer':'LogAvgMSEFastObserver'}  # 纯DFQ
    }[quantizer_name]
...
class QuantMLPBlock(QuantizedBlock):
    def __init__(self, org_module: MLPBlock, w_qconfig, a_qconfig, qinput=True, mora_config=None):
        super().__init__()
        self.lin1 = PreQuantizedLayer(org_module.lin1, None, w_qconfig, a_qconfig, mora_config=mora_config)
        self.lin2 = PreQuantizedLayer(org_module.lin2, None, w_qconfig, a_qconfig, mora_config=mora_config)
...
class QuantDecoderOurTwoWayAttentionBlock(nn.Module):
    ...
    def __init__(self, org_module: TwoWayAttentionBlock, w_qconfig, a_qconfig, ptq4sam_config, qoutput=True ) -> None:
        ...
        self.mlp = QuantMLPBlock(org_module.mlp, w_qconfig, a_qconfig, qinput=True, mora_config=getattr(ptq4sam_config, 'mora_config', None))
...
class QuantDecoderOurAttentionBlock(QuantizedBlock):
    def __init__(self, org_module: DecoderAttention, w_qconfig, a_qconfig, ptq4sam_config, qoutput=True, qinput=False):
        ...
        # Softmax量化器选择：优先DIAGQ > AGQ > 默认
        if hasattr(ptq4sam_config, 'DIAGQ') and ptq4sam_config.DIAGQ:
            softmax_a_config = update_specialized_quantizer_config(a_qconfig, 'diagq')
        elif hasattr(ptq4sam_config, 'USE_DFQ') and ptq4sam_config.USE_DFQ:
            softmax_a_config = update_specialized_quantizer_config(a_qconfig, 'dfq')
...
class QunatEncoderOurBlock(nn.Module):
    def __init__(self, org_module: Block, w_qconfig, a_qconfig, ptq4sam_config, qoutput=True ) -> None:
        ...
        # print(self.attn)
        self.norm2 = org_module.norm2
        self.mlp = QuantMLPBlock(org_module.mlp, w_qconfig, a_qconfig, mora_config=getattr(ptq4sam_config, 'mora_config', None))
