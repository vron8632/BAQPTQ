...
# 导入新的动态区间量化器
from .dynamic_interval_quantizer import (
    DynamicIntervalAdaptiveGranularityQuantizer,
    SimpleDFQQuantizer
)
...
class DIAGQFakeQuantize(QuantizeBase):
    """
    动态区间自适应粒度量化器 (DIAGQ) 的Fake Quantize包装
    融合AIQViT的DFQ和PTQ4SAM的AGQ
    
    主要用于Softmax后的非均匀分布量化
    """

    def __init__(self, observer, bit=8, symmetric=False, ch_axis=-1, 
                 enable_interval_learning=True, tau_candidates=None):
        super().__init__(observer, bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        
        # 创建内部量化器
        if tau_candidates is None:
            tau_candidates = [1.5, 2.0, 2.5, 3.0]  # 默认搜索范围
        
        self.diagq = DynamicIntervalAdaptiveGranularityQuantizer(
            n_bits=bit,
            low_org=5e-4,  # Softmax后分布的下界
            tau_candidates=tau_candidates,
            enable_interval_learning=enable_interval_learning
        )
        
        self.value = None  # 用于存储value矩阵（如果需要）
...
class SimpleDFQFakeQuantize(QuantizeBase):
    """
    简化版DFQ量化器的Fake Quantize包装
    仅使用动态区间，不使用AGQ
    """
