...
import argparse
import os
# 在任何MMCV相关导入前设置，强制禁用所有CUDA扩展
os.environ['MMCV_WITH_OPS'] = '0'         # 完全禁用MMCV CUDA算子
os.environ['MMCV_NMS_BACKEND'] = 'cpu'    # 强制使用CPU版本NMS
os.environ['MMCV_CUDA_OPS'] = '0'         # 禁用MMCV CUDA算子
os.environ['FORCE_CUDA'] = '0'            # 强制不使用CUDA编译
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'  # 同步CUDA操作便于调试
# 设置其他CUDA相关环境变量以避免兼容性问题
try:
    import torch
    # 如果torch可用，检查CUDA
    ...
    print(f"CUDA检查出错: {cuda_error}，使用CPU模式")
...
try:
    ...
    print("pip install mmcv-full -f https://download.openmmlab.com/mmcv/dist/cpu/torch1.10.0/index.html")
    import sys
    sys.exit(1)
...
try:
    ...
    from mmdet.utils import (build_ddp, build_dp, compat_cfg, get_device,
                             ...
                             update_data_root)
    print("MMDetection模块导入成功")
except ImportError as e:
    print(f"MMDetection导入失败: {e}")
    print("请检查MMDetection安装:")
    print("cd mmdetection && python setup.py develop")
    import sys
    sys.exit(1)
import sys
...
def parse_args():
    ...
    parser.add_argument(
        '--q_config',
        type=str,
        default='./exp/config66_mora.yaml',
...
def main():
    ...
    cali_data = utils.load_calibration(cfg, distributed, q_config.calibrate)    # list:32
        ...
        if len(args.load_sam_path) == 0:
            ...
            if hasattr(q_config, 'recon'):
                ...
                try:
                    ...
                    # 设置测试数据
                    from recon import set_test_data
                    set_test_data(data_loader, single_gpu_test)
                except ImportError:
                    logger.warning("Could not import test functions, intermediate testing disabled")
            # 重建优化
            if hasattr(q_config, 'recon'):
                if rank == 0:
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            metric = dataset.evaluate(outputs, **eval_kwargs)      # 计算mAP等评估指标
            print(metric)
    ...
    def replace_module(module, w_qconfig, a_qconfig, ptq4sam_config, qoutput=True):
        for name, child_module in module.named_children():
            ...
            if type(child_module) in specials:
                ...
                # 传递MoRA配置到QuantizedLayer
                mora_config = getattr(ptq4sam_config, 'mora_config', None)
                setattr(module, name, QuantizedLayer(child_module, None, w_qconfig, a_qconfig, qoutput, mora_config=mora_config))
...
def calibrate(model, cali_data, BIG):
    """校准函数 - 使用与原始ptq4sam/solver/test_quant.py相同的逻辑"""
    st = time.time()
